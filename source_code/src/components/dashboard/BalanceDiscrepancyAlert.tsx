...
const BalanceDiscrepancyAlert: React.FC = () => {
    const { accounts, savings, expenses, cards, fixedIncomes, adjustSavings } = useFinance();

    const result = useMemo(
        () => calculateBalanceDiscrepancy(accounts, savings, expenses, cards),
        [accounts, savings, expenses, cards]
    );
                ...
                <div style={{ borderTop: `1px solid ${borderColor}`, padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    ...
                    {!isOverdraft && (
                        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '12px', padding: '0.9rem 1rem' }}>
                            ...
                            {compromisoGastos > 0.005 && <BreakdownRow label="Gastos pendientes del mes (cuenta/efectivo/débito)" value={-compromisoGastos} color="#f87171" />}
