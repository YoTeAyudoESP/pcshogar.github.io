...
class IncomeDB {
    ...
    async adjustSavingGoalWithTransaction(
        ...
        await allocStore.add({
            ...
            goalId,
            amount: amount,
            type: customType || (isBudgetAdjustment ? 'manual' : 'adjustment'),
            description: customDescription || (amount >= 0 ? (isBudgetAdjustment ? 'Aportación manual' : 'Ajuste de saldo') : 'Retirada de fondos'),
            date: targetDate,
            updatedAt: Date.now(),
            budgetMonth,
            budgetYear
