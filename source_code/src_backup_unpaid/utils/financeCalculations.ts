...
export function calculateAvailableBalanceForMonth(
    ...
    if (activeOverride) {
        ...
        const incomeAfter = [...fixedIncomes, ...extraIncomes].filter(inc => {
            ...
            return isItemInMonthAndYear(inc, month, year) && d > overrideTime;
        ...
        const expensesAfter = expenses.filter(exp => {
            ...
            const d = Number(new Date(rawDate));
            return isItemInMonthAndYear(exp, month, year) && d > overrideTime;
        }).reduce((sum, exp) => sum + exp.amount, 0);

        const allocationsAfter = allocations.filter(alloc => {
            const rawDate = alloc.date || alloc.updatedAt || 0;
            const d = Number(new Date(rawDate));
            return isItemInMonthAndYear(alloc, month, year) && d > overrideTime && (alloc.type === 'manual' || alloc.type === 'automatic');
        }).reduce((sum, alloc) => sum + alloc.amount, 0);

        return {
            availableToSpend: activeOverride.amount + incomeAfter - expensesAfter - allocationsAfter - pendingFixedExpenses - pendingSavings,
            totalMonthIncome,
            totalMonthExpenses,
