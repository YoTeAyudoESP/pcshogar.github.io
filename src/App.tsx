...
import { APP_THEMES } from './types/finance';

function AppContent() {
  const { isAuthenticated, settings } = useAppSettings();

  useEffect(() => {
    const root = document.documentElement;
    
    // Default theme uses the hardcoded variables in index.css
    if (!settings.theme || settings.theme === 'default') {
      ...
      root.removeAttribute('data-theme');
      const selectedTheme = APP_THEMES.find(t => t.id === settings.theme);
      if (selectedTheme) {
