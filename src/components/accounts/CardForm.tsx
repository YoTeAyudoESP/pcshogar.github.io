...
const CardForm: React.FC<CardFormProps> = ({ onClose, editingCard, onCancelEdit }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="glass-panel" style={{ 
            ...
            <div style={{ 
                ...
                <button 
                    ...
                    style={{ 
                        flex: 1, 
                        padding: '0.8rem', 
                        background: type === 'virtual' ? 'var(--color-success)' : 'transparent', 
                        border: 'none', 
                        borderRadius: '0.5rem', 
                        color: 'var(--text-main)',
                        fontWeight: 600,
            ...
            <div>
                <label style={{ display: 'block', marginBottom: '0.75rem', color: 'rgba(var(--color-rgb-light), 0.6)', fontSize: '0.9rem' }}>Nombre</label>
                <input style={inputStyle} value={name} onChange={e => setName(e.target.value)} placeholder="ej. Tarjeta Virtual Prepago" required />
            </div>

            {type !== 'virtual' && (
                <div>
                    <label style={{ display: 'block', marginBottom: '0.75rem', color: 'rgba(var(--color-rgb-light), 0.6)', fontSize: '0.9rem' }}>Cuenta Asociada</label>
            ...
            {type === 'credit' && (
                <>
                    <div>
                        <label style={{ display: 'block', marginBottom: '0.75rem', color: 'rgba(var(--color-rgb-light), 0.6)', fontSize: '0.9rem' }}>Límite de Crédito</label>
                        <input type="number" style={inputStyle} value={limit} onChange={e => setLimit(e.target.value)} placeholder="3000" />
                    </div>
                    <div style={{ display: 'flex', gap: '1.5rem' }}>
                        <div style={{ flex: 1 }}>
                            <label style={{ display: 'block', marginBottom: '0.75rem', color: 'rgba(var(--color-rgb-light), 0.6)', fontSize: '0.9rem' }}>Día Cierre</label>
                            <input type="number" min="1" max="31" style={inputStyle} value={cutoffDay} onChange={e => setCutoffDay(e.target.value)} />
                        </div>
                        <div style={{ flex: 1 }}>
                            <label style={{ display: 'block', marginBottom: '0.75rem', color: 'rgba(var(--color-rgb-light), 0.6)', fontSize: '0.9rem' }}>Día Pago</label>
            ...
            <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1rem' }}>
                ...
                <button type="submit" style={{
                    ...
                    borderRadius: '1rem',
                    border: 'none',
                    background: type === 'credit' ? 'var(--color-primary)' : type === 'virtual' ? 'var(--color-success)' : '#EC4899',
                    color: 'var(--text-main)',
                    fontWeight: 700,
                    fontSize: '1.1rem',
                    cursor: 'pointer',
                    boxShadow: `0 4px 15px ${type === 'credit' ? 'rgba(124, 58, 237, 0.4)' : type === 'virtual' ? 'rgba(var(--color-success-rgb), 0.4)' : 'rgba(236, 72, 153, 0.4)'}`,
                    transition: 'all 0.2s ease'
                }}>
