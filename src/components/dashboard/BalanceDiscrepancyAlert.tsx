...
const BreakdownRow: React.FC<{
    ...
    separator?: boolean;
    bold?: boolean;
}> = ({ label, value, color = 'rgba(var(--color-rgb-light),0.7)', separator = false, bold = false }) => {
    if (Math.abs(value) < 0.005 && !bold) return null;
    return (
        <>
            {separator && <div style={{ borderTop: '1px solid var(--panel-border)', margin: '4px 0' }} />}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '3px 0' }}>
                <span style={{ fontSize: '0.82rem', color: bold ? 'rgba(var(--color-rgb-light),0.85)' : 'rgba(var(--color-rgb-light),0.5)', fontWeight: bold ? 700 : 400 }}>{label}</span>
        ...
        <div style={{ borderRadius: '16px', border: `1px solid ${borderColor}`, background: bgColor, marginBottom: '1rem', overflow: 'hidden', transition: 'all 0.3s ease' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                    {isOverdraft && (
                        <>
                            ...
                            <div style={{ fontSize: '0.8rem', color: 'rgba(var(--color-rgb-light),0.55)', marginTop: '2px' }}>Tu saldo real en bancos y efectivo es negativo. Toca para ver más.</div>
                        </>
                    )}
                    {isUnassigned && (
                        <>
                            <div style={{ fontWeight: 700, color: accentColor, fontSize: '0.95rem' }}>💰 Tienes {formatMoney(desajuste)} libres sin asignar</div>
                            <div style={{ fontSize: '0.8rem', color: 'rgba(var(--color-rgb-light),0.55)', marginTop: '2px' }}>Dinero real disponible tras descontar compromisos del mes y huchas.</div>
                        </>
                    )}
                    {isNegativeGap && (
                        <>
                            ...
                            <div style={{ fontSize: '0.8rem', color: 'rgba(var(--color-rgb-light),0.55)', marginTop: '2px' }}>Lo asignado excede tu dinero libre real. Toca para ajustar.</div>
                        </>
                    )}
                    {/* Real-date badge — always visible */}
                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginTop: '5px' }}>
                        <Calendar size={11} color="rgba(var(--color-rgb-light),0.3)" />
                        <span style={{ fontSize: '0.72rem', color: 'rgba(var(--color-rgb-light),0.3)', fontWeight: 500 }}>
                            Estado real · {monthLabel}
                        </span>
                    </div>
                </div>

                <div style={{ color: 'rgba(var(--color-rgb-light),0.4)', flexShrink: 0 }}>
            ...
            {expanded && (
                <div style={{ borderTop: `1px solid ${borderColor}`, padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    ...
                    {!isOverdraft && (
                        <div style={{ background: 'rgba(var(--color-rgb-light),0.03)', border: '1px solid var(--panel-border)', borderRadius: '12px', padding: '0.9rem 1rem' }}>
                            <BreakdownRow label="Saldo bruto bancos / efectivo" value={dineroReal} color="rgba(var(--color-rgb-light),0.75)" />
                            ...
                            <BreakdownRow label="Dinero libre real" value={dineroLibreReal} color={dineroLibreReal >= 0 ? 'var(--color-success)' : '#f43f5e'} separator bold />
                    ...
                    {isOverdraft && (
                        <>
                            ...
                            <p style={{ margin: 0, fontSize: '0.85rem', color: 'rgba(var(--color-rgb-light),0.65)', lineHeight: 1.5 }}>
                            ...
                            {showProjection && incomeProjection.length > 0 && (
                                <div style={{ background: 'rgba(var(--color-rgb-light),0.03)', border: '1px solid var(--panel-border)', borderRadius: '12px', overflow: 'hidden' }}>
                                    <div style={{ padding: '0.75rem 1rem', fontSize: '0.8rem', color: 'rgba(var(--color-rgb-light),0.4)', borderBottom: '1px solid var(--panel-bg-2)', fontWeight: 600 }}>PRÓXIMOS INGRESOS FIJOS (MENSUAL)</div>
                                    {incomeProjection.map((row, i) => (
                                        <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.75rem 1rem', borderBottom: i < incomeProjection.length - 1 ? '1px solid var(--panel-border)' : undefined, background: row.solves ? 'rgba(var(--color-success-rgb),0.06)' : undefined }}>
                                            <div>
                                                <div style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-main)' }}>{row.name}</div>
                                                <div style={{ fontSize: '0.75rem', color: 'rgba(var(--color-rgb-light),0.45)' }}>{formatDate(row.date)}</div>
                                            </div>
                                            <div style={{ textAlign: 'right' }}>
                                                <div style={{ fontSize: '0.9rem', fontWeight: 700, color: 'var(--color-success)' }}>+{formatMoney(row.amount)}</div>
                                                <div style={{ fontSize: '0.75rem', color: row.solves ? 'var(--color-success)' : 'rgba(var(--color-rgb-light),0.45)' }}>Acum.: {formatMoney(row.cumulative)}{row.solves && ' ✓'}</div>
                                            </div>
                                        </div>
                                    ))}
                                    {incomeProjection.some(r => r.solves) && (
                                        <div style={{ padding: '0.75rem 1rem', fontSize: '0.8rem', color: 'var(--color-success)', fontWeight: 600, borderTop: '1px solid rgba(var(--color-success-rgb),0.15)' }}>
                            ...
                            {incomeProjection.length === 0 && (
                                <p style={{ margin: 0, fontSize: '0.8rem', color: 'rgba(var(--color-rgb-light),0.4)', fontStyle: 'italic' }}>
                    ...
                    {isUnassigned && (
                        <>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                                ...
                                <div style={statBox}><span style={statLabel}>Pendiente de asignar</span><span style={{ ...statValue, color: remainingToAssign > 0.005 ? '#f59e0b' : 'var(--color-success)' }}>{formatMoney(remainingToAssign)}</span></div>
                            </div>
                            <p style={{ margin: 0, fontSize: '0.85rem', color: 'rgba(var(--color-rgb-light),0.65)', lineHeight: 1.5 }}>
                            ...
                            {savings.length > 0 ? (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                                    <div style={{ fontSize: '0.8rem', color: 'rgba(var(--color-rgb-light),0.4)', fontWeight: 600 }}>ASIGNAR A HUCHAS</div>
                                    {savings.map((goal: SavingGoal) => (
                                        <div key={goal.id} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                            ...
                                            <span style={{ flex: 1, fontSize: '0.85rem', color: 'rgba(var(--color-rgb-light),0.8)' }}>{goal.name}</span>
                                ...
                                <p style={{ margin: 0, fontSize: '0.85rem', color: 'rgba(var(--color-rgb-light),0.4)', fontStyle: 'italic' }}>No tienes huchas configuradas. Ve a Gestión y Ajustes → Huchas para crear una.</p>
                            )}
                            {savings.length > 0 && (
                                <button onClick={handleApplyDistribution} disabled={distributing || totalDistributed < 0.005}
                                    style={{ ...applyButton, background: totalDistributed < 0.005 ? 'var(--panel-bg-2)' : 'rgba(245,158,11,0.2)', border: `1px solid ${totalDistributed < 0.005 ? 'rgba(var(--color-rgb-light),0.08)' : 'rgba(245,158,11,0.35)'}`, color: totalDistributed < 0.005 ? 'rgba(var(--color-rgb-light),0.3)' : '#f59e0b', cursor: totalDistributed < 0.005 ? 'not-allowed' : 'pointer' }}>
                                    <Check size={16} />
                    ...
                    {isNegativeGap && (
                        <>
                            ...
                            {savings.filter(s => s.currentAmount > 0).length > 0 ? (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                                    <div style={{ fontSize: '0.8rem', color: 'rgba(var(--color-rgb-light),0.4)', fontWeight: 600 }}>REDUCIR HUCHAS</div>
                                    {savings.filter(s => s.currentAmount > 0).map((goal: SavingGoal) => (
                                        <div key={goal.id} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                            <PiggyBank size={16} color="#818cf8" style={{ flexShrink: 0 }} />
                                            <div style={{ flex: 1 }}>
                                                <div style={{ fontSize: '0.85rem', color: 'rgba(var(--color-rgb-light),0.8)' }}>{goal.name}</div>
                                                <div style={{ fontSize: '0.72rem', color: 'rgba(var(--color-rgb-light),0.35)' }}>Saldo actual: {formatMoney(goal.currentAmount)}</div>
                                            </div>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                                ...
                                <p style={{ margin: 0, fontSize: '0.85rem', color: 'rgba(var(--color-rgb-light),0.4)', fontStyle: 'italic' }}>No tienes huchas con saldo para ajustar.</p>
                            )}
                            {savings.filter(s => s.currentAmount > 0).length > 0 && (() => {
                                ...
                                return (
                                    <button onClick={handleApplyReduction} disabled={reducing || !isReductionValid}
                                        style={{ ...applyButton, background: !isReductionValid ? 'var(--panel-bg-2)' : 'rgba(251,146,60,0.15)', border: `1px solid ${!isReductionValid ? 'rgba(var(--color-rgb-light),0.08)' : 'rgba(251,146,60,0.35)'}`, color: !isReductionValid ? 'rgba(var(--color-rgb-light),0.3)' : '#fb923c', cursor: !isReductionValid ? 'not-allowed' : 'pointer' }}>
                                        <Check size={16} />
...
const statBox: React.CSSProperties = { background: 'rgba(var(--color-rgb-light),0.03)', border: '1px solid var(--panel-border)', borderRadius: '10px', padding: '0.75rem 1rem', display: 'flex', flexDirection: 'column', gap: '4px' };
const statLabel: React.CSSProperties = { fontSize: '0.75rem', color: 'rgba(var(--color-rgb-light),0.4)', fontWeight: 500 };
const statValue: React.CSSProperties = { fontSize: '1.1rem', fontWeight: 800 };
const linkButton: React.CSSProperties = { background: 'none', border: 'none', color: 'rgba(var(--color-rgb-light),0.55)', cursor: 'pointer', fontSize: '0.85rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '6px', padding: 0, textDecoration: 'underline', textDecorationColor: 'rgba(var(--color-rgb-light),0.25)', textUnderlineOffset: '3px' };
const numInput: React.CSSProperties = { width: '80px', padding: '6px 8px', borderRadius: '8px', border: '1px solid var(--panel-bg-3)', background: 'var(--panel-bg-2)', color: 'var(--text-main)', fontSize: '0.85rem', textAlign: 'right', outline: 'none' };
...
const dismissButton: React.CSSProperties = { flex: 1, padding: '0.5rem', background: 'var(--panel-bg-2)', border: '1px solid var(--panel-bg-3)', borderRadius: '8px', color: 'rgba(var(--color-rgb-light),0.7)', fontSize: '0.8rem', cursor: 'pointer', transition: 'all 0.2s ease' };
