    ...
    return (
        ...
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-container glass-panel" style={{ 
                ...
                <button 
                    onClick={onClose}
                    style={{ position: 'absolute', top: '1.5rem', right: '1.5rem', background: 'var(--panel-bg-2)', border: 'none', color: 'rgba(var(--color-rgb-light),0.5)', cursor: 'pointer', padding: '8px', borderRadius: '50%' }}
                >
                    <X size={20} />
                </button>
                ...
                <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                    ...
                    <div>
                        ...
                        <div style={{ position: 'relative' }}>
                            ...
                            <Calendar 
                                size={18} 
                                style={{ position: 'absolute', right: '1rem', top: '1.6rem', color: 'rgba(var(--color-rgb-light),0.3)', pointerEvents: 'none' }} 
                            />
                        </div>
                    </div>
                        ...
                        <>
                            ...
                            {paymentMethodType !== 'cash' && (
                             ...
                             <div style={{ 
                                 ...
                                 <label style={{ display: 'flex', alignItems: 'center', gap: '12px', cursor: 'pointer', color: 'var(--text-main)', fontSize: '1rem', fontWeight: 600 }}>
                                     <input 
                                         type="checkbox" 
                                 ...
                                 <p style={{ color: 'rgba(var(--color-rgb-light),0.4)', fontSize: '0.8rem', marginTop: '6px', marginLeft: '30px' }}>
                                                     ...
                                                     .map(([id, val]) => {
                                                         ...
                                                         return (
                                                             <div key={id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem' }}>
                                                                 <span style={{ color: 'rgba(var(--color-rgb-light),0.7)' }}>{h ? h.name : 'Hucha'}</span>
                                                                 <span style={{ color: 'var(--text-main)', fontWeight: 600 }}>{formatMoney(parseFloat(val))}</span>
                                                             </div>
                                                         );
                                                     })}
                                                 {totalAllocated === 0 && (
                                                     <span style={{ fontSize: '0.85rem', color: 'rgba(var(--color-rgb-light),0.4)', fontStyle: 'italic' }}>
                                                         Ninguna hucha financia este gasto todavía. Pulsa en 'Ajustar importes'.
                                                     </span>
                                                 )}
                                             </div>

                                             <div style={{ borderTop: '1px solid var(--panel-border)', paddingTop: '0.5rem', display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem' }}>
                                                 <span style={{ color: 'rgba(var(--color-rgb-light),0.5)' }}>Total financiado:</span>
                    ...
                    <button type="submit" disabled={isFundingInvalid} style={{
                        ...
                        color: isFundingInvalid ? 'rgba(var(--color-rgb-light),0.3)' : 'var(--text-main)',
                        fontWeight: 700,
                        fontSize: '1.1rem',
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                    <div>
                        ...
                        <div style={{ fontSize: '0.85rem', color: 'rgba(var(--color-rgb-light),0.5)', marginTop: '4px' }}>
                            Total Gasto: <strong style={{ color: 'var(--text-main)' }}>{formatMoney(expenseTotal)}</strong>
                        </div>
                    </div>
                    <button 
                        type="button" 
                ...
                <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '1rem', paddingBottom: '1rem' }}>
                    {savings.length === 0 ? (
                        <p style={{ color: 'rgba(var(--color-rgb-light),0.4)', textAlign: 'center', marginTop: '2rem' }}>No tienes huchas creadas.</p>
                    ) : (
                        savings.map(h => {
                            ...
                            return (
                                <div 
                                    key={h.id} 
                                    style={{
                                        background: isChecked ? 'rgba(99, 102, 241, 0.12)' : 'rgba(var(--color-rgb-light),0.03)',
                                        border: isChecked ? '2px solid #6366f1' : '1px solid var(--panel-border)',
                                        borderRadius: '14px',
                                        padding: '1rem',
                                        display: 'flex',
                                        flexDirection: 'column',
                                ...
                                >
                                    <label style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', margin: 0 }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                            ...
                                            />
                                            <span style={{ fontSize: '1.05rem', fontWeight: 700, color: 'var(--text-main)' }}>{h.name}</span>
                                        </div>
                                        <span style={{ fontSize: '0.9rem', color: 'rgba(var(--color-rgb-light),0.5)', fontWeight: 600 }}>
                                            Disponible: <span style={{ color: 'var(--color-success)' }}>{formatMoney(available)}</span>
                                        </span>
                                    </label>
                ...
                {/* Footer */}
                <div style={{ 
                    borderTop: '1px solid var(--panel-bg-3)', 
                    paddingTop: '1.25rem',
                    display: 'flex',
                    flexDirection: 'column',
                    gap: '1rem',
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span style={{ fontSize: '0.95rem', color: 'rgba(var(--color-rgb-light),0.6)' }}>Total Asignado:</span>
                        <span style={{ 
                            fontSize: '1.25rem', 
                            fontWeight: 800, 
