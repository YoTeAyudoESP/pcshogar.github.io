        ...
        <div style={{
                ...
                <p style={{
                    ...
                    {pendingNextDayExpenses.length === 1 ? (
                        <>
                            ...
                            <strong style={{ color: 'var(--text-main)' }}>{formatMoney(pendingNextDayExpenses[0].amount)}</strong>.
                        </>
                    ) : (
                        <>
                            Mañana se prevé el cobro de <strong style={{ color: 'var(--text-main)' }}>{pendingNextDayExpenses.length} pagos fijos</strong> por un importe total de{' '}
                            <strong style={{ color: 'var(--text-main)' }}>{formatMoney(totalAmount)}</strong>: {pendingNextDayExpenses.map((e, index) => (
                                <span key={e.id}>
            ...
            <button 
                ...
                onMouseEnter={e => {
                    ...
                    e.currentTarget.style.backgroundColor = 'var(--panel-bg-2)';
                }}
                onMouseLeave={e => {
                    e.currentTarget.style.color = 'rgba(var(--color-rgb-light),0.4)';
