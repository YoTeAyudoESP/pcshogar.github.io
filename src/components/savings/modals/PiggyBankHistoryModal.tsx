...
const PiggyBankHistoryModal: React.FC<PiggyBankHistoryModalProps> = ({ goal, onClose }) => {
    ...
    const getIcon = (type: SavingAllocation['type'], amount: number) => {
        if (type === 'transfer_in') return <ArrowDownLeft size={16} color="var(--color-success)" />;
        if (type === 'transfer_out') return <ArrowUpRight size={16} color="#ef4444" />;
        if (amount >= 0) return <ArrowDownLeft size={16} color="var(--color-success)" />;
    ...
    return (
        <div style={{
            ...
            <div className="glass-panel" style={{ 
                ...
                <div style={{ 
                    padding: '1.5rem', borderBottom: '1px solid var(--panel-bg-2)',
                    ...
                    <button 
                        onClick={onClose}
                        style={{ background: 'none', border: 'none', color: 'rgba(var(--color-rgb-light),0.3)', cursor: 'pointer', padding: '8px' }}
                    >
                        <X size={24} />
                    </button>
                </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                            {history.map(item => (
                                <div 
                                    ...
                                    style={{
                                        display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                                        padding: '1rem', background: 'var(--panel-bg-1)', borderRadius: '12px',
                                ...
                                >
                                    ...
                                    <div style={{ 
                                        fontWeight: 700, 
                                        fontSize: '1rem',
                                        color: item.amount >= 0 ? 'var(--color-success)' : '#f43f5e'
                                    }}>
                ...
                <div style={{ padding: '1.5rem', background: 'rgba(0,0,0,0.1)', borderBottomLeftRadius: '1.5rem', borderBottomRightRadius: '1.5rem', textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--text-main)' }}>{formatMoney(goal.currentAmount)}</div>
                </div>
            </div>
        </div>
    );
