...
const FixedIncomeForm: React.FC<FixedIncomeFormProps> = ({ editingIncome, onClose, onNavigateToSettings }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="glass-panel" style={{ padding: '1.75rem', animation: 'slideDown 0.3s ease-out', maxWidth: '500px', margin: '0 auto' }}>
            ...
            <div style={{ ...containerStyle, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem', background: 'rgba(25, 27, 34, 0.4)', borderRadius: '0.75rem', border: '1px solid var(--panel-bg-3)' }}>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <span style={{ fontSize: '0.95rem', fontWeight: 500 }}>Contabilizar al mes siguiente</span>
                    <span style={{ fontSize: '0.8rem', color: 'rgba(var(--color-rgb-light),0.5)' }}>El cobro se asignará automáticamente al presupuesto del mes próximo</span>
                </div>
            ...
            <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
                ...
                <button type="submit" disabled={hasNoAccounts} style={{
                    ...
                    color: hasNoAccounts ? 'rgba(var(--color-rgb-light),0.3)' : 'var(--text-main)',
                    fontWeight: 700,
                    fontSize: '1.1rem',
