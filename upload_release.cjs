...
async function main() {
    ...
    let res = await request('POST', 'https://api.github.com/repos/' + OWNER + '//' + REPO + '/releases'.replace('//', '/'), JSON.stringify(releaseData), {
        'Content-Type': 'application/json'
    });
    
    if (res.errors && res.errors[0] && res.errors[0].code === 'already_exists') {
        console.log('Release already exists, getting it...');
        res = await request('GET', 'https://api.github.com/repos/' + OWNER + '//' + REPO + '/releases/tags/' + TAG + ''.replace('//', '/'));
    } else {
        console.log('RES:', res);
    }
